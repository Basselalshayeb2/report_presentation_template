...
\begin{frame}{Описание задачи}
  \begin{itemize}
    ...
    \item \textbf{Цель данной работы:} улучшить систему, внедрив интерфейс голосового управления, который позволит врачам выполнять ключевые действия при помощи голосовых команд.
...
\begin{frame}{Матрица противоречий (Часть 1)}
  ...
  \begin{itemize}
    \item \textbf{21. Принцип проскока (Skipping)}: обрабoтка наиболее важных сегментов аудио и пропуск неинформативных аудиосигналов.
    \item \textbf{11. Принцип "заранее подложенной подушки" (Cushion in advance)}: предусмотреть буфер, чтобы не потерять начало команды.
    \item \textbf{26. Принцип копирования (Copying)}: использовать упрощённую «копию» основной модели для предварительного анализа.
    \item \textbf{31. Принцип применения пористых материалов (Porous materials)}%: чередовать интервалы активного и пассивного распознавания.
  \end{itemize}
...
\begin{frame}{Матрица противоречий (Часть 2)}
  \begin{block}{27 (Надёжность) vs 36 (Сложность)}
    ...
    \begin{itemize}
      \item \textbf{13. Принцип "наоборот" (The other way round)}%: включать и выключать сложные компоненты в зависимости от состояния}.
      \item \textbf{35. Принцип изменения параметров (Parameter change)}: динамически менять частоту дискретизации.
      \item \textbf{1. Принцип дробления (Segmentation)}: разделить систему на фазы: Idle → Trigger → Recognition.
    \end{itemize}
  \end{frame}

\begin{frame}{Выбранные изобретательские принципы и идея решения}
  \begin{enumerate}
    \item \textbf{26 – Принцип копирования:} простая модель для предварительного распознавания.
    \item \textbf{21 – Принцип проскока:} игнорирование неинформативных аудиосигналов и фоновых шумов.
    \item \textbf{11 – Принцип "заранее подложенной подушки":} предусмотреть буфер, чтобы не потерять начало команды.
    \item \textbf{35 – Принцип изменения параметров:} адаптивная смена настроек (частота дискретизации).
    \item \textbf{1 – Принцип дробления:} разделить систему на фазы.
    % \item \textbf{13 – Обратная связь:} автоматическое включение/отключение сложных модулей в зависимости от активности.
  \end{enumerate}

  \bigskip
  \textbf{Изобретательская идея:}
  создать адаптивную систему распознавания речи, в которой простая модель предварительно фильтрует аудио и активирует полную модель нейросети только при необходимости.
\end{frame}

\begin{frame}{Карта соответствия: принципы ТРИЗ \(\rightarrow\) архитектура голосового помощника}
  \centering
  \includegraphics[width=\textwidth]{triz/state_diagram.jpg}
  \vspace{2mm}
...
%   % === Wrap up ===
  ...
  \begin{enumerate}
    ...
    \item Выбранные принципы ТРИЗ: \textbf{26}, \textbf{21}, \textbf{11}, \textbf{35}, \textbf{1}.
    \item Изобретательская идея: VAD/KWS → ASR.
  \end{enumerate}
  \end{frame}
