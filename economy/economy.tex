...
\begin{frame}{Решение: MyGym Finance}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item Мобильное приложение: \textbf{бюджет} + \textbf{тренировки} + \textbf{аналитика ценности}.
        \item ИИ-план на месяц: как \textbf{сэкономить 15–25\%} без потери качества тренировок.
        \item Интеграции: абонемент, CRM клуба, кэшбэк-партнёры.
        \item Напоминания: «Смените на квартальный — экономия \good{20\%}».
        \item \textbf{Геймификация}: очки за дисциплину, челленджи.
      \end{itemize}
    \end{column}
...
\begin{frame}{Рынок и аудитория}
  \begin{itemize}
    ...
    \item Тренд: \textbf{Digital Wellness} и финтех растут двузначными темпами.
    \item TAM $\sim$ рынок фитнеса ($\approx\$80$ млрд), SAM — цифровой велнесс ($\approx\$10$ млрд), SOM — локальные рынки СНГ/Европы.
  \end{itemize}
  \vspace{0.6em}
  \begin{tikzpicture}[every node/.style={font=\small}]
    \node[draw,rounded corners,inner sep=6pt,fill=MyLight] (tam) {TAM: Фитнес $ \approx \$80$ млрд};
...
\begin{frame}{Конкуренты и наше преимущество}
  ...
  \begin{block}{\good{Moat} (барьеры копирования)}
...
\begin{frame}{Бизнес-модель и ценообразование}
  \textbf{Freemium}: базовые функции бесплатно, \textbf{Premium} — \good{299 ₽/мес}. \par
  B2B для клубов: \good{1500 ₽/локацию/мес}.
  ...
  \begin{block}{Пример value-case}
    Средний пользователь экономит \good{600–1200 ₽/мес} → Premium \good{окупается 2–4x}.
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \begin{itemize}
        ...
        \item Выручка: \good{~3.0 млн ₽}. B2B: \good{~0.9 млн ₽}.
        \item Итого: \good{~3.9 млн ₽} (без учёта комиссий).
      \end{itemize}
    \end{column}
    \begin{column}{0.44\textwidth}
      \textbf{Запрос: \good{0.8–1.2 млн ₽}} \newline
      на маркетинг и масштабирование.
      \medskip

      \begin{block}{Окупаемость}
        Цель: \good{ROI 150\%} за 18 мес (ориентир).
      \end{block}
    \end{column}
   \end{columns}
...
\begin{frame}{Почему инвестировать в MyGym Finance}
  ...
  \vspace{0.5em}
  \begin{center}
    \Large \good{Тренируйся умно — трать ещё умнее.}
  \end{center}
\end{frame}
